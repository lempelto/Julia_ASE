...
"""
function create_units(codata_version)
    u = Dict{String,Number}
    try
        u = CODATA[codata_version]
    catch e
        error("CODATA version '$codata_version' not implemented")
    end
...
# Add the constants to the module scope:
_units = create_units(__codata_version__)
for _unit in _units
    _first = Symbol(_unit.first)
    _second = _unit.second
    _ex = :($_first = $_second) # Expression for <unit> = <value>
    eval(_ex) # Evaluate expression
end
